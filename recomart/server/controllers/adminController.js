...
const getSellers = async (req, res, next) => {
  try {
    ...
    const sellers = await VendorProfile.find(filter)
      .populate('user', 'fullName email phone avatar isActive createdAt')
      .sort({ createdAt: -1 });

    return ApiResponse.success(res, { sellers }, 'Sellers retrieved successfully');
  } catch (error) {
    next(error);
  }
};
...
const getReports = async (req, res, next) => {
  try {
    ...
    const [revenueByMonth, userGrowth, ordersByStatus, topCategories] = await Promise.all([
      Payment.aggregate([
